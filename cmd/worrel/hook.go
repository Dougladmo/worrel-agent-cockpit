...
func runHookCmd(args []string) {
	if len(args) == 0 || args[0] != "prompt" {
		fmt.Fprintln(os.Stderr, "uso: worrel hook prompt --session <id> --port <port> [--format claude|codex|gemini]")
		os.Exit(2)
	}
	fs := flag.NewFlagSet("hook prompt", flag.ExitOnError)
	session := fs.String("session", "", "id da sessão")
	port := fs.Int("port", 0, "porta do worrel")
	format := fs.String("format", "claude", "formato de saída: claude|codex|gemini")
	_ = fs.Parse(args[1:])
